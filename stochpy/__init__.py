...
def load_files_slab2(zone='south_america',rake=False):
    ...
    ## rake
    if rake==True:
        slabrake   = np.genfromtxt(directory+'sam_rake.xyz', delimiter = ",")
...
def estima_rigidez_prem(profs ):
    ...
    prem_prof = PREM.depth.to_numpy() * 1000 # profundidad en metros
...
def magnitud_momento(slip, prof, lons, lats):
    ...
    Mo = np.sum(Mo)
    Mw = 2.0/3.0*np.log10(Mo)-6.05

    return Mw, area_subfalla

# estima la magnitud de momento utilizando la rigidez obtenida del modelo PREM

def magnitud_momento_PREM(slip, prof, lons, lats):
    """
    Calcula la magnitud de momento Mw de la distribucion de slip con la ecuacion
    Mw = 2/3log(Mo)-6.05. Calcula la rigidez individual de cada subfalla dependiendo de
    ...
    rigidez = estima_rigidez_prem(prof)
...
def media_slip(Mw,largo,ancho,prof):
    """
    Largo, ancho y profundidad deben estar en metros
    """
    rigidez_media=np.mean(estima_rigidez_prem(prof).flatten())
    Mo=10**((Mw+2/3*9.1)*3/2)
    media_slip=Mo/(largo*ancho*rigidez_media)
    return np.trunc(media_slip)
# escala la magnitud
def escalar_magnitud_momento(Mw, slip, prof, lons, lats,prem=False):
    """
    ...
    if prem==False:    
        ...
        Mw_prim, areasf = magnitud_momento_PREM( slip, prof, lons, lats ) # magnitud de momento de dist slip primitiva
    Mo_original     = 10**( (3./2.) * ( Mw_prim + 6.05 ) ) # momento de la distribucion original
    Mo_deseado      = 10**( (3./2.) * ( Mw + 6.05 ) )  # momento deseado para la magnitud a escalar
...
def tukey_window(N, alpha=0.5):
    ...
    if alpha <= 0:
        ...
        return w
def taper_except_trench_tukey(Slip,dip_taperfunc=tukey_window,strike_taperfunc=windows.tukey,alpha_dip=0.5,alpha_strike=0.5):
    """
...
def plot_slip(X_grid,Y_grid,lonfosa,latfosa,Slip,filename,show=False,cmap='rainbow'):
        ...
        if show==False:
            ...
            return plt.show()
def plot_slip_gmt(region,X_grid,Y_grid,lonfosa,latfosa,Slip,dx,dy,archivo_salida):
    """
    ...
    fig=pygmt.Figure()
    spacing=f"{dx}k/{dy}k"
    spacing2=f"{0.003*dx}k/{0.003*dy}k"
    cmap2use = 'rainbow'
    ...
    fig.plot(x=lonfosa,y=latfosa,
        ...
        fill="white",
        style="f0.5i/0.1i+r+t+o1")
    fig.colorbar(frame="x+lSlip[m]",cmap=cmap)
